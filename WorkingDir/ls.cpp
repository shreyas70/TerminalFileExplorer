...
vector<string> cmdTokensAbsolutePath;
 ...
 for (substring = strtok(c_cmd, sep); substring; substring = strtok(NULL, sep), i++)
 {
   ...
   if(cmdTokens[0].compare("copy")==0){
        for(int i=1;i<cmdTokens.size()-1;i++){
            //appending file name to destination directory to create file
            string destination = cmdTokensAbsolutePath[cmdTokensAbsolutePath.size()-1]+"/"+cmdTokens[i];
            ...
            strcpy(source, cmdTokensAbsolutePath[i].c_str());
...
void convToAbsolute(){ //updated the vector cmdTokens
    cmdTokensAbsolutePath.push_back("cmd"); //to match the indices b/w cmdToken and this
    for(int i=1;i<cmdTokens.size();i++){
        ...
        if(temp[0]== '/'){
            ...
            cmdTokensAbsolutePath.push_back(root_string+cmdTokens[i]);

        }
        else if(temp[0]=='~'){
            char* t1=getRoot();
            string root_string =string(t1);
            cmdTokensAbsolutePath.push_back(root_string+cmdTokens[i].substr(1));


        }
        else if(temp[0]=='.'){
            string currentDir =  dirHistory.list[dirHistory.curIndex];
            cmdTokensAbsolutePath.push_back(currentDir+cmdTokens[i].substr(1));


        }
        else{
            string currentDir = dirHistory.list[dirHistory.curIndex];
            cmdTokensAbsolutePath.push_back(currentDir+"/"+cmdTokens[i]);
